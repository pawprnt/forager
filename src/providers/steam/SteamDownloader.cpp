...
{
    return paths::depotDownloaderDir() + "/DepotDownloader";
}
...
void SteamDownloader::requireConfigured() const
{
    if (!SteamCredentials::instance().hasCredentials()) {
                               ...
                               ProgressFn onProgress, std::atomic<bool>* cancel)
{
    ...
    auto r = subprocess::runStreaming(depotDownloaderPath(), args, cancel, 200,
